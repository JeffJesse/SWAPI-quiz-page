...
async function transformPersonaje(data) {
  ...
  const film = transformPelicula(filmData.title);
...
async function transformPlaneta(data) {
  ...
  const filmResponse = await fetch(data.films[0]);
  const filmData = await filmResponse.json();
  film = transformPelicula(filmData.title);
...
function transformPelicula(pelicula) {
  switch (pelicula) {
    case "A New Hope":
      return "Una nueva esperanza";
    case "The Phantom Menace":
      return "La amenaza fantasma";
    case "The Empire Strikes Back":
      return "El imperio contraataca";
    case "Return of the Jedi":
      return "El regreso del Jedi";
    default:
      break;
  }
}
...
function formatPopulation(population) {
  ...
  let formattedPopulation =
    parseInt(population).toLocaleString() + " Habitantes";
